...
class MediapipeDetect(Detect):
    def return_detection_result(self,model_path:str,frame:np.ndarray):
    ...
    def return_landmarks(self,model_path:str,frame:np.ndarray) -> list:
        """returns a list of landmark points on a body with their
            coordinates in x,y,z for a single frame (the frame should be in BRG)"""
        detection_result = self.return_detection_result(model_path,frame)    
        
        return detection_result.pose_landmarks
      
    def return_world_landmarks(self,model_path:str,frame:np.ndarray) -> list:
